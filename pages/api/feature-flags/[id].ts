...
export default async function handler(req: any, res: any) {
  try {
    ...
    if (req.method === 'PUT') {
      ...
      if (!canManage) {
        return res.status(403).json({ message: "Access denied: Cannot update feature flags" });
      }
    ...
    if (req.method === 'DELETE') {
      ...
      const canManage = await canManageFeatureFlags(user);
      if (!canManage) {
        return res.status(403).json({ message: "Access denied: Cannot delete feature flags" });
      }
