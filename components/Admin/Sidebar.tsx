...
class Sidebar extends Component<ISidebarProps, ISidebarState> {
  constructor(props: ISidebarProps) {
    super(props);
    this.state = {
      displays: [], // Initialize with empty, fetched in componentDidMount
    };
  }
  ...
  async componentDidMount() {
    ...
    try {
      ...
      this.setState({
        displays: displaysData.map(d => ({ _id: d._id, name: d.name })),
      });
    } catch (error) {
  ...
  navigateToAdmin = (id: string): void => {
    ...
    displayStore.setId(id); // Update the global store
  };
  ...
  render() {
    ...
    // Use displayStore.id for constructing menu paths if available, otherwise fallback or handle
    const currentDisplayId = this.props.displayId || displayStore.id || (displays.length > 0 ? displays[0]._id : '');
    ...
    return (
      <div className='sidebar'>
        {loggedIn && (
          ...
          >
            <div className='logo'>
              ...
              <div className='info'>
                <span className='name'>{displayStore.name || 'Select Display'}</span>
        ...
        {loggedIn && (
          <div className='logout' onClick={this.handleLogout} role="button" tabIndex={0} onKeyPress={(e) => {if(e.key === 'Enter' || e.key === ' ') this.handleLogout()}}>
