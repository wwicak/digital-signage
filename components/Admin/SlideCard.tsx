...
import SlideEditDialog, { ISlideEditDialog } from './SlideEditDialog'; // Assuming SlideEditDialog.tsx and its ref interface
...
interface ISlideCardState {
  loading: boolean; // For delete operation
}

class SlideCard extends Component<ISlideCardProps, ISlideCardState> {
  private dialog = React.createRef<ISlideEditDialog>();

  constructor(props: ISlideCardProps) {
    super(props);
    this.state = {
      loading: false,
    };
  ...
  handleDelete = (): void => {
    ...
    deleteSlide(value._id)
      .then(() => {
        refresh();
        // No need to setState loading false if component is unmounted by refresh
        // But if refresh doesn't unmount, then:
  ...
  getThumbnailIcon = (slideType: string): IconProp => {
    switch (slideType) {
  ...
  getThumbnailBackgroundColor = (slideType: string): string => {
    switch (slideType) {
      case 'youtube':
      case 'video':
  ...
  render() {
    ...
    const showBackgroundImage = slideType === 'photo' || slideType === 'image';
    const thumbnailStyle: React.CSSProperties = {
      backgroundImage: showBackgroundImage && value.data ? `url(${value.data})` : 'none',
      backgroundColor: this.getThumbnailBackgroundColor(slideType),
    };
      ...
      <div className='slide-card'>
        ...
        <div className='left-content'>
          <div className={'thumbnail'} style={thumbnailStyle}>
            {!showBackgroundImage && (
              <FontAwesomeIcon icon={this.getThumbnailIcon(slideType)} fixedWidth size='lg' color='#FFFFFF' />
            )}
          </div>
        </div>
        ...
        <div className='right-actions'>
          <div className='action-icon' onClick={this.handleEdit} role="button" tabIndex={0} onKeyPress={(e) => {if(e.key === 'Enter' || e.key === ' ') this.handleEdit()}} aria-label="Edit slide">
            <FontAwesomeIcon icon={faEdit} fixedWidth color='#828282' />
          </div>
          <div className='action-icon' onClick={!loading ? this.handleDelete : undefined} role="button" tabIndex={!loading ? 0 : -1} onKeyPress={(e) => {if(!loading && (e.key === 'Enter' || e.key === ' ')) this.handleDelete()}} aria-label="Delete slide" aria-disabled={loading}>
            <FontAwesomeIcon
              icon={faTrash}
              fixedWidth
        ...
        <SlideEditDialog ref={this.dialog} slideId={value._id} refresh={refresh} />
        <style jsx>
          {`
            .slide-card {
              padding: 12px;
