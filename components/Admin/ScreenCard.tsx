...
// interface IScreenCardState {}

class ScreenCard extends Component<IScreenCardProps> {
  handleDelete = (event: React.MouseEvent<SVGSVGElement>): void => {
  ...
  render() {
    ...
    return (
      ...
      <Link href={`/layout?display=${value._id}`} passHref legacyBehavior>
        <a className='card-anchor-wrapper'> {/* Use an anchor tag for proper semantics with Next.js Link */}
          <div className='card'>
            ...
            <div className='right'>
              ...
              {/* View Display Link */}
              <Link href={`/display/${value._id}`} passHref legacyBehavior>
              ...
              {/* Delete Action */}
              <div className='actionIcon' onClick={(e: React.MouseEvent<HTMLDivElement>) => {
                  // Need to cast event target if strict event types are needed for FontAwesomeIcon's onClick
                  this.handleDelete(e as any); 
                }}
                role="button" 
                tabIndex={0} 
                onKeyPress={(e: React.KeyboardEvent<HTMLDivElement>) => { if (e.key === 'Enter' || e.key === ' ') this.handleDelete(e as any);}}
