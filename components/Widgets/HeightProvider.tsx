...
export default function HeightProvider<P extends object>(
  ComposedComponent: ComponentType<
    P & { width: number; rowHeight: number; cols: number }
  >,
  ...
  return class HeightProviderComponent extends React.Component<
    P & HeightProviderProps,
    HeightProviderState
  > {
    ...
    onWindowResize = () => {
      ...
      if (node instanceof HTMLElement) {
        ...
        if (
          Math.abs(newWidth - this.state.width) > 5 ||
          Math.abs(newHeight - this.state.height) > 5
        ) {
    ...
    render() {
      ...
      const rowNum =
        layout.length > 0
          ? Math.max(
              1,
              ...layout.map((widget: WidgetLayout) => widget.y + widget.h),
            )
      ...
      const colNum =
        layout.length > 0
          ? Math.max(
              1,
              ...layout.map((widget: WidgetLayout) => widget.x + widget.w),
            )
          : 12; // Default to 12
