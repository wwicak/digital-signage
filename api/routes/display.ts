...
interface UpdateDisplayBody
  extends Partial<
    Omit<
      IDisplay,
      "_id" | "creator_id" | "creation_date" | "last_update" | "widgets"
    >
  > {
...
router.get(
  ...
  async (req: Request<{ id: string }>, res: Response) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ message: "Error fetching display.", error: err.message });
    }
  }
);
...
router.post(
  "/",
  ensureAuthenticated,
  async (req: Request<{}, any, CreateDisplayBody>, res: Response) => {
    const user = req.user as IUser;
    if (!user || !user._id) {
      res.status(400).json({ message: "User information not found." });
      return;
    ...
    try {
      if (widgetsData && widgetsData.length > 0) {
        ...
        const typedWidgetsData = widgetsData.map((w) => ({
          ...w,
          type: w.type as WidgetType,
        }));
        await createWidgetsForDisplay(
          newDisplayDoc,
          typedWidgetsData,
          user._id as string
        );
      ...
      sendEventToDisplay(
        (savedDisplay._id as mongoose.Types.ObjectId).toString(),
        "display_updated",
        {
          displayId: (savedDisplay._id as mongoose.Types.ObjectId).toString(),
          action: "create",
        }
      );
      ...
      res
        .status(500)
        .json({ message: "Error creating display", error: error.message });
    }
  }
);
...
router.put(
  ...
  ensureAuthenticated,
  async (
    req: Request<{ id: string }, any, UpdateDisplayBody>,
    res: Response
  ) => {
    const user = req.user as IUser;
    if (!user || !user._id) {
    ...
    try {
      ...
      res
        .status(500)
        .json({ message: "Error updating display", error: error.message });
    }
  }
);
...
router.delete(
  ...
  async (req: Request<{ id: string }>, res: Response) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ message: "Error deleting display", error: error.message });
    }
  }
);
