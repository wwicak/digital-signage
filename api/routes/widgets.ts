...
router.post(
  "/",
  ensureAuthenticated,
  async (req: Request<{}, any, CreateWidgetBody>, res: Response) => {
    const user = req.user as IUser;
    if (!user || !user._id) {
      res.status(400).json({ message: "User information not found." });
      return;
...
router.put(
  ...
  ensureAuthenticated,
  async (
    req: Request<{ id: string }, any, UpdateWidgetBody>,
    res: Response
  ) => {
    const user = req.user as IUser;
    if (!user || !user._id) {
    ...
    try {
      ...
      try {
        const displayIds = await getDisplayIdsForWidget(
          savedWidget._id as mongoose.Types.ObjectId
        );
        for (const displayId of displayIds) {
          sendEventToDisplay(displayId, "display_updated", {
            ...
            widgetId: (savedWidget._id as mongoose.Types.ObjectId).toString(),
          });
        }
      } catch (notifyError) {
        console.error(
          `Error notifying displays after widget update ${widgetId}:`,
          notifyError
        );
