    ...
    try {
      ...
      try {
        const displayIds = await getDisplayIdsForSlide(
          savedSlide._id as mongoose.Types.ObjectId
        );
        for (const displayId of displayIds) {
          sendEventToDisplay(displayId, "display_updated", {
            ...
            action: "update",
            reason: "slide_change",
            slideId: (savedSlide._id as mongoose.Types.ObjectId).toString(),
          });
        }
      } catch (notifyError) {
        console.error(
          `Error notifying displays after slide update ${slideId}:`,
          notifyError
        );
