...
router.get('/:id', ensureAuthenticated, async (req: Request<{ id: string }>, res: Response) => {
  ...
  try {
    const slide = await Slide.findOne({ _id: req.params.id, creator_id: user._id });
    if (!slide) {
...
router.post('/', ensureAuthenticated, async (req: Request<{}, any, CreateSlideBody>, res: Response) => {
  const user = req.user as IUser;
  if (!user || !user._id) {
    res.status(400).json({ message: 'User information not found.' });
    return;
  ...
  const { name, description, type, data, duration, is_enabled, slideshow_ids } = req.body;

  if (!name || !type || data === undefined) {
    res.status(400).json({ message: 'Slide name, type, and data are required.' });
    return;
  }
...
router.put('/:id', ensureAuthenticated, async (req: Request<{ id: string }, any, UpdateSlideBody>, res: Response) => {
  const user = req.user as IUser;
  if (!user || !user._id) {
    res.status(400).json({ message: 'User information not found.' });
    return;
  ...
  try {
    const slideToUpdate = await Slide.findOne({ _id: slideId, creator_id: user._id });
    ...
    if (slideshow_ids !== undefined) { 
        ...
        originalSlideshowIds = slideshowsContainingSlide.map(s => s._id);
    }
    ...
    if (slideshow_ids !== undefined) {
      await handleSlideInSlideshows(savedSlide, slideshow_ids, originalSlideshowIds);
    }
...
router.delete('/:id', ensureAuthenticated, async (req: Request<{ id: string }>, res: Response) => {
  ...
  try {
    ...
    if (!deletedSlide) { 
        res.status(404).json({ message: 'Slide not found during deletion process.' });
        return;
    }
