    ...
    try {
      ...
      try {
        const displayIds = await getDisplayIdsForSlideshow(
          savedSlideshow._id as mongoose.Types.ObjectId
        );
        ...
        console.error(
          `Error notifying displays after slideshow update ${slideshowId}:`,
          notifyError
        );
  ...
  async (req: Request<{ id: string }>, res: Response) => {
    ...
    try {
      ...
      try {
        ...
        console.error(
          `Error fetching display IDs before slideshow delete ${slideshowId}:`,
          notifyError
        );
