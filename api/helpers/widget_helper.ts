 ...
 */
export const validateWidgetData = async (type: WidgetType, data: any): Promise<boolean> => {
  if (!data) {
  ...
  switch (type) {
    case WidgetType.ANNOUNCEMENT:
      if (typeof data.title !== 'string' || typeof data.message !== 'string') {
        throw new Error('Invalid data for Announcement widget: title and message must be strings.');
      }
      break;
    case WidgetType.CONGRATS:
      if (typeof data.title !== 'string' || typeof data.message !== 'string' || typeof data.recipient !== 'string') {
        throw new Error('Invalid data for Congrats widget: title, message, and recipient must be strings.');
      }
      break;
    case WidgetType.IMAGE:
      if (typeof data.url !== 'string' || !data.url.match(/\.(jpeg|jpg|gif|png)$/)) {
        throw new Error('Invalid data for Image widget: URL must be a valid image URL (jpeg, jpg, gif, png).');
      }
      break;
    case WidgetType.LIST:
      if (!Array.isArray(data.items) || !data.items.every((item: any) => typeof item === 'string')) { // Added type for item
        throw new Error('Invalid data for List widget: items must be an array of strings.');
      }
      if (data.title && typeof data.title !== 'string') {
        throw new Error('Invalid data for List widget: title, if provided, must be a string.');
      }
      break;
    case WidgetType.SLIDESHOW:
      if (!data.slideshow_id || !mongoose.Types.ObjectId.isValid(data.slideshow_id)) {
        throw new Error('Invalid data for Slideshow widget: slideshow_id must be a valid ObjectId string.');
      }
    ...
    case WidgetType.WEATHER:
      ...
      if (typeof data.location !== 'string' && (typeof data.location !== 'object' || !data.location.city)) {
         throw new Error('Invalid data for Weather widget: location (string or object with city) is required.');
      }
      ...
      break;
    case WidgetType.WEB:
      if (typeof data.url !== 'string' || !data.url.startsWith('http')) { // Basic URL check
        throw new Error('Invalid data for Web widget: URL must be a valid web URL.');
      }
      break;
    case WidgetType.YOUTUBE:
      if (typeof data.video_id !== 'string') { // Could also validate with a regex for YouTube video IDs
 ...
 */
export const removeWidgetFromAllDisplays = async (widgetId: string | mongoose.Types.ObjectId): Promise<void> => {
  const idToRemove = typeof widgetId === 'string' ? new mongoose.Types.ObjectId(widgetId) : widgetId;
...
export const deleteWidgetAndCleanReferences = async (
  ...
  const idToDelete = typeof widgetId === 'string' ? new mongoose.Types.ObjectId(widgetId) : widgetId;
  ...
  try {
    ...
    throw new Error(`Failed to delete widget ${idToDelete} and clean references.`);
  }
};
