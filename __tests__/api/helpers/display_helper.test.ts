...
const MockedWidgetConstructor = jest.fn().mockImplementation((constructorData: any) => {
  const originalMongooseInternal = jest.requireActual('mongoose');
  const idForThisInstance = constructorData && constructorData._id ? new originalMongooseInternal.Types.ObjectId(constructorData._id) : new originalMongooseInternal.Types.ObjectId();
  const instanceData = {
  ...
  if (constructorData && constructorData.name === 'New Widget For Error Test' && constructorData.shouldFailSave) {
    currentSaveMock.mockRejectedValueOnce(new Error('New widget save failed'));
  }
...
MockedWidgetConstructor.find = jest.fn().mockReturnValue({ exec: jest.fn().mockResolvedValue([]) });
MockedWidgetConstructor.findById = jest.fn().mockReturnValue({ exec: jest.fn().mockResolvedValue(null) });
MockedWidgetConstructor.insertMany = jest.fn().mockResolvedValue([]);
MockedWidgetConstructor.findOne = jest.fn().mockReturnValue({ exec: jest.fn().mockResolvedValue(null) });
...
MockedDisplayFindByIdQuery_populateFn.mockImplementation(() => MockedDisplayFindByIdQuery);
...
jest.mock('mongoose', () => {
  ...
  return {
    ...
    model: jest.fn((name, schema) => {
      ...
      const GenericMockModel = jest.fn(data => ({ ...data, save: jest.fn().mockResolvedValue(data), toJSON: jest.fn().mockReturnValue(data) }));
      GenericMockModel.find = jest.fn().mockReturnValue({ exec: jest.fn().mockResolvedValue([]) });
      GenericMockModel.findById = jest.fn().mockReturnValue({ exec: jest.fn().mockResolvedValue(null) });
      GenericMockModel.findByIdAndUpdate = jest.fn().mockImplementation((id, update) => ({ ...update, _id: id }));
      GenericMockModel.deleteMany = jest.fn().mockResolvedValue({ acknowledged: true, deletedCount: 0 });
      GenericMockModel.insertMany = jest.fn().mockResolvedValue([]);
      GenericMockModel.findOne = jest.fn().mockReturnValue({ exec: jest.fn().mockResolvedValue(null) });
      GenericMockModel.create = jest.fn(async (data: any) => ({ ...data, _id: new originalMongoose.Types.ObjectId() }));
...
import { createWidgetsForDisplay, updateWidgetsForDisplay, getDisplayWithWidgets } from '../../../api/helpers/display_helper';
...
describe('updateWidgetsForDisplay', () => {
    ...
    describe('Error Handling', () => {
      ...
      beforeEach(() => {
        ...
        (MockedWidgetConstructor.findByIdAndUpdate as jest.Mock).mockImplementation(async (id, dataToUpdate) => {
            const originalMongooseInternal = jest.requireActual('mongoose');
            const objectId = typeof id === 'string' ? new originalMongooseInternal.Types.ObjectId(id) : id;
            // Simulate findByIdAndUpdate returning the updated document
            return { ...dataToUpdate, _id: objectId, widgets: currentDisplayObject.widgets };
        });
        (MockedWidgetConstructor.deleteMany as jest.Mock).mockResolvedValue({ acknowledged: true, deletedCount: 1 });
        (MockedWidgetConstructor.create as jest.Mock).mockImplementation(async (dataToCreate: any) => {
            const originalMongooseInternal = jest.requireActual('mongoose');
            // Ensure created object has an _id and other necessary fields
            const newId = new originalMongooseInternal.Types.ObjectId();
            return { ...dataToCreate, _id: newId, save: jest.fn().mockResolvedValue({ ...dataToCreate, _id: newId }), toJSON: jest.fn().mockReturnValue({ ...dataToCreate, _id: newId }) };
        });
        ...
        MockedWidgetConstructor.mockImplementation((constructorData: any) => {
            ...
            if (constructorData && constructorData.name === 'New Widget For Error Test' && constructorData.shouldFailSave) {
                currentSave = jest.fn().mockRejectedValue(new Error('New widget save failed'));
            }
            return {
              ...instanceData,
              save: currentSave,
      ...
      it('Error during new widget creation: should process other operations and log error', async () => {
        ...
        const resultWidgetObjectIds = await updateWidgetsForDisplay(currentDisplayObject as any, mixedWidgetsData, creatorId, MockedWidgetConstructor as any);
      ...
      it('Error during widget update: should process other operations and log error', async () => {
        ...
        (MockedWidgetConstructor.findByIdAndUpdate as jest.Mock).mockRejectedValueOnce(updateError);

        const resultWidgetObjectIds = await updateWidgetsForDisplay(currentDisplayObject as any, newWidgetsDataArray, creatorId, MockedWidgetConstructor as any);
        ...
        const newWidgetInstanceCall = MockedWidgetConstructor.mock.calls.find(callArgs => callArgs[0].name === newWidgetData.name);
        ...
        for(let i=0; i < results.length; i++){
            if(results[i].type === 'return' && results[i].value.name === newWidgetData.name){
        ...
        expect(resultWidgetObjectIds.map(id => id.toString()).sort()).toEqual([createdNewWidgetId!].sort());
        expect(currentDisplayObject.widgets).toEqual(initialDisplayWidgetObjectIds);
      });
      ...
      it('Error during widget deletion: should process other operations and log error', async () => {
        ...
        (MockedWidgetConstructor.deleteMany as jest.Mock).mockRejectedValueOnce(deleteError);
        ...
        const expectedIds = [createdNewWidgetId!, widgetToUpdateId.toString()].sort();
...
describe('deleteWidgetsForDisplay', () => {
  ...
  beforeEach(() => {
    ...
    if (MockedWidgetConstructor && (MockedWidgetConstructor.deleteMany as jest.Mock)?.mockReset) {
        (MockedWidgetConstructor.deleteMany as jest.Mock).mockReset();
    }
  ...
  it('should successfully delete widgets and empty the display widgets array', async () => {
    ...
    (MockedWidgetConstructor.deleteMany as jest.Mock).mockResolvedValue({ acknowledged: true, deletedCount: originalWidgetIds.length });

    // Import deleteWidgetsForDisplay locally if not available globally or handle imports carefully
    const { deleteWidgetsForDisplay } = await import('../../../api/helpers/display_helper');
    await deleteWidgetsForDisplay(displayObject);

    expect(MockedWidgetConstructor.deleteMany).toHaveBeenCalledTimes(1);
    expect(MockedWidgetConstructor.deleteMany).toHaveBeenCalledWith({ _id: { $in: originalWidgetIds } }); // Use original IDs
  ...
  it('should throw an error if Widget.deleteMany fails', async () => {
    ...
    (MockedWidgetConstructor.deleteMany as jest.Mock).mockRejectedValue(dbError);

    const { deleteWidgetsForDisplay } = await import('../../../api/helpers/display_helper');
    await expect(deleteWidgetsForDisplay(displayObject))
      .rejects
      .toThrow('Failed to delete widgets for display.');

    expect(MockedWidgetConstructor.deleteMany).toHaveBeenCalledTimes(1);
    expect(MockedWidgetConstructor.deleteMany).toHaveBeenCalledWith({ _id: { $in: displayObject.widgets } });
    expect(consoleErrorSpy).toHaveBeenCalledWith('Error deleting widgets for display:', dbError);
...
describe('getDisplayWithWidgets', () => {
  ...
  it('should return a display with populated widgets if found', async () => {
    ...
    (MockedDisplayConstructor.findById as jest.Mock).mockResolvedValue(mockDisplayDoc);
    ...
    expect(MockedDisplayConstructor.findById).toHaveBeenCalledWith(mockDisplayId.toString());
  ...
  it('should return null and log error if database query fails', async () => {
    ...
    // We no longer assert populate and exec calls
    expect(result).toBeNull();
    expect(consoleErrorSpy).toHaveBeenCalledWith('Error fetching display with widgets:', dbError);
  });

  it('should handle displayId as ObjectId', async () => {
    const mockDisplayDoc = { _id: mockDisplayId, name: 'Test Display', widgets: [] };
    (MockedDisplayConstructor.findById as jest.Mock).mockResolvedValue(mockDisplayDoc);

    await getDisplayWithWidgets(mockDisplayId); // Pass ObjectId directly

    expect(MockedDisplayConstructor.findById).toHaveBeenCalledWith(mockDisplayId);
  });
