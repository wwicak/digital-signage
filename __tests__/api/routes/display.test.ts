...
describe("Display API Routes", () => {
  ...
  describe("POST /", () => {
    ...
    it("should create a new display with default orientation when not specified", async () => {
      const { orientation, ...dataWithoutOrientation } = newDisplayData;
      const mockSavedDisplay = {
        ...dataWithoutOrientation,
        _id: savedDisplayId,
        creator_id: mockUser._id,
        orientation: "landscape", // Should default to landscape
        widgets: [],
        populate: jest.fn().mockResolvedValue({
          ...dataWithoutOrientation,
          _id: savedDisplayId,
      ...
      const response = await request(app)
        .post("/api/v1/displays")
        .send(dataWithoutOrientation);

      expect(response.status).toBe(201);
      expect(response.body.orientation).toBe("landscape");
      expect(mockSavedDisplay.populate).toHaveBeenCalledWith("widgets");
    });
