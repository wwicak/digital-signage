...
const ensureAuthenticatedMock = (req: any, res: Response, next: NextFunction) => {
  req.user = mockUser;
...
describe('Slide API Routes', () => {
  ...
  describe('GET /', () => {
    it('should use findAllAndSend helper to fetch all slides', async () => {
      ...
      const mockedFindAllAndSendDirect = commonHelpers.findAllAndSend as jest.Mock;
      mockedFindAllAndSendDirect.mockImplementation((model, res, query, projection) => {
      ...
      expect(mockedFindAllAndSendDirect).toHaveBeenCalledWith(Slide, expect.any(Object), undefined, { creator_id: mockUser._id });
    });
  ...
  describe('POST /', () => {
    ...
    it('should create a new slide successfully', async () => {
      const savedSlideMock = { ...slideData, _id: 'slideNew', creator_id: mockUser._id };
      ...
      expect(mockedHandleSlideInSlideshows).toHaveBeenCalledWith(savedSlideMock, slideData.slideshow_ids, []);
    });
    ...
    it('should return 400 if required fields (name, type, data) are missing', async () => {
      ...
      const response = await request(app)
        ...
        .send(incompleteData);
      expect(response.status).toBe(400);
      expect(response.body.message).toBe('Slide name, type, and data are required.');
    });
    ...
    it('should return 400 on validation error (e.g., from Mongoose)', async () => {
      ...
      expect(response.body.errors).toEqual({ name: { message: 'Name is required' } });
    });
    ...
    it('should call handleSlideInSlideshows with empty array if slideshow_ids is not provided', async () => {
      ...
      const savedSlideMock = { ...dataWithoutSlideshows, _id: 'slideNew', creator_id: mockUser._id };
      slideSaveSpy.mockResolvedValue(savedSlideMock as any);
      mockedHandleSlideInSlideshows.mockResolvedValue(undefined);

      await request(app)
        .post('/api/v1/slides')
        .send(dataWithoutSlideshows);
  ...
  describe('GET /:id', () => {
    it('should fetch a specific slide successfully', async () => {
      const mockSlide = { _id: 'slide1', name: 'Slide 1', creator_id: mockUser._id, type: SlideType.IMAGE, data: { url: 'http://example.com/image.png' } };
      ...
      expect(slideFindOneSpy).toHaveBeenCalledWith({ _id: 'slide1', creator_id: mockUser._id });
    });
  ...
  describe('PUT /:id', () => {
    ...
    const updateData: any = { name: 'Updated Slide Name', type: SlideType.VIDEO, data: { url: 'http://new.com/vid.mp4' } };
    const initialSlide = {
      _id: slideId,
      name: 'Old Name',
      type: SlideType.IMAGE,
    ...
    it('should update a slide successfully without changing slideshows', async () => {
      ...
      expect(response.status).toBe(200);
      expect(slideFindOneSpy).toHaveBeenCalledWith({ _id: slideId, creator_id: mockUser._id });
    ...
    it('should update a slide and its slideshow associations', async () => {
      const updatedDataWithSlideshows = { ...updateData, slideshow_ids: ['newShow1'] };
      ...
      expect(slideFindOneSpy).toHaveBeenCalledWith({ _id: slideId, creator_id: mockUser._id });
      expect(initialSlide.save).toHaveBeenCalledTimes(1);
      expect(slideshowFindSpy).toHaveBeenCalledWith({ slides: initialSlide._id });
  ...
  describe('DELETE /:id', () => {
    ...
    it('should delete a slide successfully', async () => {
      ...
      mockedDeleteSlideAndCleanReferences.mockResolvedValue(mockSlideInstance as any); // Assuming it returns the deleted slide
    ...
    it('should return 404 if deleteSlideAndCleanReferences does not return a slide (e.g. already deleted)', async () => {
      ...
      expect(response.status).toBe(404);
      expect(response.body.message).toBe('Slide not found during deletion process.');
    });
    ...
    it('should return 500 if deleteSlideAndCleanReferences helper throws an error', async () => {
      slideFindOneSpy.mockResolvedValue(mockSlideInstance as any);
      mockedDeleteSlideAndCleanReferences.mockRejectedValue(new Error('Helper error'));
      const response = await request(app)
        .delete(`/api/v1/slides/${slideId}`);
      expect(response.status).toBe(500);
      expect(response.body.message).toBe('Error deleting slide');
