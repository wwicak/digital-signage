...
describe('Slideshow API Routes', () => {
  ...
  const mockedValidateSlidesExist = slideshowHelper.validateSlidesExist as jest.Mock;
  const mockedReorderSlidesInSlideshow = slideshowHelper.reorderSlidesInSlideshow as jest.Mock;
  const mockedPopulateSlideshowSlides = slideshowHelper.populateSlideshowSlides as jest.Mock;
  ...
  describe('GET /:id', () => {
    ...
    it('should return 400 if user information is not found for GET /:id', async () => {
      ...
      const response = await request(tempApp).get(`/api/v1/slideshows/${slideshowId}`);
      expect(response.status).toBe(400);
      expect(response.body.message).toBe('User information not found.');
    });
  });
  ...
  describe('POST /', () => {
    ...
    const savedFullSlideshowMock = { ...fullSlideshowData, _id: 'fullShowId', creator_id: mockUser._id, slides: fullSlideshowData.slide_ids };
    ...
    it('should create a new slideshow successfully (minimal valid payload - only name)', async () => {
      ...
      mockedPopulateSlideshowSlides.mockImplementation(async (ss) => ({...ss, description: ss.description, is_enabled: ss.is_enabled, slides: ss.slides || []}));
    ...
    it('should create a new slideshow successfully (full payload)', async () => {
      ...
      mockedPopulateSlideshowSlides.mockResolvedValue(savedFullSlideshowMock as any);
      ...
      expect(mockedValidateSlidesExist).toHaveBeenCalledWith(fullSlideshowData.slide_ids);
      expect(slideshowProtoSaveSpy).toHaveBeenCalledTimes(1);
      expect(mockedPopulateSlideshowSlides).toHaveBeenCalledWith(savedFullSlideshowMock);
    });
    ...
    it('should return 400 if Zod validation fails for slide_ids (not valid ObjectId)', async () => {
      const invalidSlideIdData = { ...fullSlideshowData, slide_ids: ['slide1Id', 'invalid-object-id-string'] };
      const response = await request(app)
        .post('/api/v1/slideshows')
      ...
      expect(response.status).toBe(400);
      expect(response.body.message).toBe('Validation failed');
      expect(response.body.errors?.slide_ids?.[1]).toContain('Invalid ObjectId in slide_ids');
    });
    ...
    it('should create slideshow with empty slides if slide_ids is not provided', async () => {
      ...
      mockedPopulateSlideshowSlides.mockImplementation(async (ss) => ({...ss, slides: ss.slides || [] }));
    ...
    it('should return 400 if user information is not found for POST /', async () => {
      ...
      const response = await request(tempApp).post('/api/v1/slideshows').send(fullSlideshowData);
      expect(response.status).toBe(400);
      expect(response.body.message).toBe('User information not found.');
    });
  });
  ...
  describe('PUT /:id', () => {
    ...
    const updatePayload = { name: 'Updated Slideshow Name', description: 'Updated desc' };
