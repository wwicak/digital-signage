...
export interface ISlideData {
  ...
  type: 'image' | 'video' | 'web' | 'markdown'; // Example types, adjust as needed
...
export const getSlides = (slideshowId: string, host: string = ''): Promise<ISlideData[]> => {
  return axios.get<ISlideData[]>(`${host}/api/v1/slideshow/${slideshowId}/slides`).then((res: AxiosResponse<ISlideData[]>) => {
    if (res && res.data) {
      return res.data;
    }
...
export const getSlide = (slideId: string, host: string = ''): Promise<ISlideData> => {
  return axios.get<ISlideData>(`${host}/api/v1/slide/${slideId}`).then((res: AxiosResponse<ISlideData>) => {
    if (res && res.data) {
      return res.data;
    }
...
export const deleteSlide = (id: string, host: string = ''): Promise<AxiosResponse<IApiResponse>> => {
  return axios.delete<IApiResponse>(`${host}/api/v1/slide/${id}`);
};

// Type for the 'data' parameter in updateSlide and addSlide, excluding file
type SlideUpdateData = Omit<Partial<ISlideData>, '_id' | 'creator_id' | 'creation_date' | 'last_update' | 'data'> & {
...
export const updateSlide = (id: string, file: File | null, data: SlideUpdateData, host: string = ''): Promise<AxiosResponse<ISlideData>> => {
...
type SlideAddData = Omit<Partial<ISlideData>, '_id' | 'creator_id' | 'creation_date' | 'last_update' | 'data' | 'slideshow_ids'> & {
    // Add other specific fields that can be sent in the FormData body
    [key: string]: any; // Allow other string keys for FormData compatibility
};

export const addSlide = (slideshowId: string, file: File | null, data: SlideAddData, host: string = ''): Promise<AxiosResponse<ISlideData>> => {
  const formData = new FormData();
  for (const key in data) {
    if (Object.prototype.hasOwnProperty.call(data, key)) {
      formData.append(key, data[key]);
...
export const standaloneUpload = (file: File, host: string = ''): Promise<AxiosResponse<IStandaloneUploadResponse>> => {
  const formData = new FormData();
  formData.append('data', file); // 'data' is the field name for the file
  return axios.post<IStandaloneUploadResponse>(`${host}/api/v1/slide/standalone_upload`, formData, {
    headers: {
      'Content-Type': 'multipart/form-data',
    },
  });
